...
def resolve_paths(
    ...
    resolved_roi_dir = roi_dir or (resolved_project_root / "data" / "processed" / "roi")
    resolved_raw_dir = raw_dir or (resolved_project_root / "data" / "raw")
...
def find_missing_roi_names(roi_paths: list[Path], raw_paths: list[Path]) -> list[str]:
    roi_name_set = {p.name for p in roi_paths}
    expected_roi_names = {f"{p.stem}_roi{p.suffix.lower()}" for p in raw_paths}
    return sorted(expected_roi_names - roi_name_set)
...
def render_debug_overlay(img_bgr: np.ndarray, metrics: Dict[str, object], out_path: Path) -> None:
...
def analyze_roi_images(
    ...
    for roi_path in roi_paths:
        ...
        try:
            ...
            status = "pass" if (coverage >= min_coverage and ratio >= min_colored_ratio) else "fail"
